        ...
        for nv, img_ in enumerate(images):
            ...
            for iextra, _mesh in enumerate(self.extra_mesh):
                if True:
                    ...
                    ### test ####
                    print('in renderer1, vert: {}'.format(vert.shape))
                    print('in renderer1, faces: {}'.format(faces.shape))
                    print('in renderer1, rot: {}'.format(rot.shape))
                    ### end #####
                    mesh.apply_transform(rot)
            ...
            for trackId, data in render_data.items():
                #print('before copy, vert: {}'.format(data['vertices'].shape))
                vert = data['vertices'].copy()
                #print('after copy, vert: {}'.format(vert.shape))
                faces = data['faces']
                print('in renderer, R: {}'.format(R))
                print('in renderer, T: {}'.format(T))
                #vert = vert @ R.T + T.T  ## don't know why this line does not work
                print('after tranformation, vert: {}'.format(vert.shape))
                if 'colors' not in data.keys():
                    # 如果使用了vid这个键，那么可视化的颜色使用vid的颜色
                    col = get_colors(data.get('vid', trackId))
                    mesh = trimesh.Trimesh(vert, faces, process=False)
                     ### test ####
                    print('in renderer2, vert: {}'.format(vert.shape))
                    print('in renderer2, faces: {}'.format(faces.shape))
                    print('in renderer2, rot: {}'.format(rot.shape))
                    ### end #####
                    mesh.apply_transform(rot)
                    material = pyrender.MetallicRoughnessMaterial(
                        metallicFactor=0.0,
